...
// ✅ FIXED: Fetch all visible menu items
$menu_sql = "
    SELECT *
    FROM menu
    WHERE stock >= 0
      AND (
        (availability_start IS NULL OR availability_start = '' OR availability_start = '00:00:00' OR availability_start <= '$currentTime')
        AND (availability_end IS NULL OR availability_end = '' OR availability_end = '00:00:00' OR availability_end >= '$currentTime')
      )
        ...
        <div class="row g-4" id="menuCards">
            <?php if($menu_result->num_rows > 0): ?>
                <?php while($row = $menu_result->fetch_assoc()):
                    ...
                    $availability = $start && $end ? "$start - $end" : ($start ? "From $start" : ($end ? "Until $end" : "All day"));
                ...
                ?>
                <div class="col-md-4 menu-card-wrapper" data-name="<?= htmlspecialchars(strtolower($row['name'])) ?>" data-category="<?= $category ?>">
